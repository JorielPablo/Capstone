...
<script>
  ...
  $(document).ready(function(){
    ...
    function check_mobile() {
        var pattern = new RegExp(/^\+?[0-9]{1,3}-?[0-9]{3}-?[0-9]{3}-?[0-9]{4}$/);
        var mobile_length = $("#mobile").val().length;

        if ($.trim($('#mobile').val()) == '') {
            $("#mobile_error_message").html("mobile number is a required field.");
            $("#mobile_error_message").show();
            $("#mobilel").addClass("is-invalid");
        } else if (!(pattern.test($("#mobile").val()))) {
            $("#mobile_error_message").html("Invalid mobile number address");
            $("#mobile_error_message").show();
            error_mobile = true;
    ...
    $('#user_form').on('submit', function(event){
      ...
      if (error_fullname == false && error_username == false && error_mobile == false && error_gender == false && error_status == false && error_password == false && error_confirm_password == false) {
